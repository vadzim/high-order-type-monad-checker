...
async function withTempFile(source: string, fn: (filePath: string) => void | Promise<void>) {
	const dir = await mkdtemp(path.join(tmpdir(), "borrow-checker-cli-"))
...
`
	await withTempFile(source, filePath => {
		...
		assert.equal(
			cleanedupOutput,
			`sample.ts:4:32 - Opaque-bound variable 'A' consumed multiple times in one path.
 1 |
 2 | type O = { __opaque: "O" };
...
sample.ts:4:38 - Opaque-bound variable 'A' consumed multiple times in one path.
 1 |
 2 | type O = { __opaque: "O" };
 3 | type C<A extends O> = [A];
 4 | type X<A extends O> = [C<A>, C<A>, C<A>];
