...
function formatDiagnosticHeader(file, line, column, message, options) {
    if (!useAnsi(options)) {
...
export function formatSourceSnippetFromOffsets(file, message, source, pos, options = {}) {
...
export function resolveDiagnosticFromOffsets(source, pos) {
 ...
 * on `anchor.line`.
 */
function formatSourceSnippet(source, anchor, options) {
    const before = options?.contextBefore ?? 3;
    const after = options?.contextAfter ?? 0;
    const tabWidth = typeof options?.tabWidth === "number" && Number.isFinite(options.tabWidth) && options.tabWidth >= 1
    ...
    for (let i = start; i <= end; i++) {
        ...
        if (i === lineIndex) {
            const anchorText = " ".repeat(expandTabsInLine(lineText.slice(0, anchor.startPos), tabWidth).length) +
                (color && ansi.lightRed) +
...
function useAnsi(options) {
    if (options?.noColor)
        return "";
    return true;
}
function gutterWidthForRange(startLine, endLine) {
    let result = Math.max(String(startLine).length, String(endLine).length);
    if (result < 3)
        result += 1;
    return result;
}
function formatGutterLine(lineNo, gutterW, sourceText, color) {
...
function blankGutter(gutterW, color) {
...
function visualWidthUpTo(line, end, tabWidth) {
    let w = 0;
...
function expandTabsInLine(line, tabWidth) {
    let out = "";
    let col = 0;
 ...
 */
export function highlightFromOffsets(source, pos) {
    const lines = splitLines(source);
    let offset = 0;
