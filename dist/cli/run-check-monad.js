...
import { buildContentGraph } from "../src/buildContentGraph.js";
import { concatContentGraphs } from "../src/concatContentGraphs.js";
import { getMonadViolations } from "../src/monadChecker.js";
import { formatGraphViolation } from "./format-graph-violation.js";
import { resolveDiagnosticFromOffsets } from "./format-source-snippet.js";
...
export async function runCli(argv, streams = console) {
    try {
...
function summarizeFiles(violations, files) {
    const stats = new Map();
...
function parseCli(argv) {
    ...
    while (i < argv.length) {
        ...
        if (token === "--snippet-lines") {
            ...
            if (!value)
                throw new EInvalidOption("Missing value for --snippet-lines.");
        ...
        if (token === "--monad") {
            ...
            if (!file || !monadInfo) {
                throw new EInvalidOption("Usage: --monad <file> <type-name>:<constructor-name>:<reader-name>:<consumer-name>");
            }
            const parts = monadInfo.split(":");
            if (parts.length !== 4) {
                throw new EInvalidOption("Invalid --monad type pair. Expected <type-name>:<constructor-name>:<reader-name>:<consumer-name>.");
            }
            monadSpecs.push({
                path: path.join(file),
                name: parts[0] ?? "",
...
function parseSnippetLines(input) {
    ...
    if (parts.length > 2)
        throw new EInvalidOption(`Invalid --snippet-lines value '${input}'.`);
    if (parts.length === 1) {
...
function parseNonNegative(raw, label) {
    if (!/^\d+$/.test(raw))
        throw new EInvalidOption(`Invalid ${label} '${raw}'. Expected non-negative integer.`);
    return Number(raw);
}
async function renderHelpText() {
    let readme = "";
