...
export function never(message = "never") {
...
function collectReferenceInfo(value, ctx, depth) {
    collectReferenceInfoInner(value, ctx, depth, new Set());
    for (const [obj, count] of ctx.occurrences) {
        if (count > 1)
            ctx.isReferenced.add(obj);
    }
}
function collectReferenceInfoInner(value, ctx, depth, expanded) {
    ...
    if (count > 1 && !ctx.refIds.has(value))
        ctx.refIds.set(value, ctx.nextRefId++);
    if (expanded.has(value))
        return;
    expanded.add(value);
    if (Array.isArray(value)) {
        for (const item of value)
            collectReferenceInfoInner(item, ctx, depth + 1, expanded);
        return;
    }
    ...
    for (const key of Object.keys(value)) {
        collectReferenceInfoInner(value[key], ctx, depth + 1, expanded);
    }
}
function stylizeRefToken(token, options) {
...
function renderValue(value, ctx, depth, options) {
    if (!isInspectableObject(value))
        return renderPrimitive(value, options);
    const refId = ctx.refIds.get(value);
    const isReferenced = ctx.isReferenced.has(value);
    const refSite = ctx.refSites.get(value);
    const shouldDefineHere = refId !== undefined &&
        isReferenced &&
        refSite !== undefined &&
        refSite.depth === depth &&
        !ctx.definedRefs.has(value);
    if (refId !== undefined && isReferenced && !shouldDefineHere)
        return stylizeRefToken(`<ref *${refId}>`, options);
    if (shouldDefineHere)
        ctx.definedRefs.add(value);
    if (Array.isArray(value)) {
    ...
    const body = renderObject(value, ctx, depth, options);
    return shouldDefineHere ? `${stylizeRefToken(`<ref *${refId}>`, options)} ${body}` : body;
}
function renderArray(value, ctx, depth, options) {
    if (value.length === 0)
        return "[]";
...
function renderObject(value, ctx, depth, options) {
...
function renderSet(value, ctx, depth, options) {
    if (value.size === 0)
        return "Set(0) []";
    const childDepth = depth + 1;
    const indent = INDENT.repeat(childDepth);
...
function renderMap(value, ctx, depth, options) {
    if (value.size === 0)
        return "Map(0) {}";
    const childDepth = depth + 1;
    const indent = INDENT.repeat(childDepth);
    const closeIndent = INDENT.repeat(depth);
    const lines = [...value].map(([key, mapValue]) => `${indent}${renderValue(key, ctx, childDepth, options)} => ${renderValue(mapValue, ctx, childDepth, options)},`);
    return `Map(${value.size}) {\n${lines.join("\n")}\n${closeIndent}}`;
}
export function inspect(value, options = {}) {
    const ctx = {
        refIds: new Map(),
        isReferenced: new Set(),
        refSites: new Map(),
        occurrences: new Map(),
        definedRefs: new Set(),
        nextRefId: 1,
...
function assertUnsupported(value, why) {
...
function encodePrimitive(value) {
    ...
    if (kind === "symbol") {
        ...
        if (key === undefined)
            assertUnsupported(value, "non-global symbol");
        return `Symbol.for(${JSON.stringify(key)})`;
    }
    if (kind === "function")
        assertUnsupported(value, "functions/classes are not supported");
...
function encodePropertyKey(key) {
    ...
    if (globalName === undefined)
        assertUnsupported(key, "non-global symbol property key");
    return `[Symbol.for(${JSON.stringify(globalName)})]`;
}
function encodePropertyValue(value, ctx) {
    if (typeof value === "function")
        assertUnsupported(value, "functions/classes are not supported");
    if (typeof value === "object" && value !== null)
        return `v${visitNode(value, ctx)}`;
    return encodePrimitive(value);
}
function visitNode(value, ctx) {
    ...
    if (Array.isArray(value)) {
        ...
        for (const item of value) {
            if (typeof item === "function")
                assertUnsupported(item, "functions/classes are not supported");
    ...
    if (value instanceof Map) {
        ...
        for (const [k, v] of value) {
            if (typeof k === "function")
                assertUnsupported(k, "functions/classes are not supported");
            if (typeof v === "function")
                assertUnsupported(v, "functions/classes are not supported");
    ...
    if (ArrayBuffer.isView(value)) {
        ...
        if (!ctorName)
            assertUnsupported(value, "unknown typed array constructor");
        ctx.nodes.push({
            id,
            kind: "typedArray",
            value: value,
    ...
    if (isPlainObject(value)) {
        ...
        for (const key of Reflect.ownKeys(value)) {
            ...
            if (typeof propValue === "function")
                assertUnsupported(propValue, "functions/classes are not supported");
            if (typeof propValue === "object" && propValue !== null)
                visitNode(propValue, ctx);
        }
        return id;
    }
    assertUnsupported(value, `unsupported object type ${value.constructor?.name ?? "unknown"}`);
}
export function serialize(value) {
    ...
    const initLines = ctx.nodes.map(node => {
        switch (node.kind) {
            ...
            case "typedArray":
                return `const v${node.id} = new ${node.ctorName}(v${node.bufferId}, ${node.value.byteOffset}, ${node.value.length})`;
        }
    });
    const assignLines = [];
        ...
        switch (node.kind) {
            case "array":
                for (const item of node.value)
                    assignLines.push(`v${node.id}.push(${encodePropertyValue(item, ctx)})`);
                break;
            case "object":
            case "nullObject":
                for (const key of Reflect.ownKeys(node.value)) {
                    ...
                    if (!desc?.enumerable)
                        continue;
                    assignLines.push(`v${node.id}${encodePropertyKey(key)} = ${encodePropertyValue(node.value[key], ctx)}`);
                }
                break;
            case "set":
                for (const item of node.value)
                    assignLines.push(`v${node.id}.add(${encodePropertyValue(item, ctx)})`);
                break;
            case "map":
